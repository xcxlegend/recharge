...
class DfpayController extends Controller
{
    ...
    public function add()
    {
        ...
        $user_id =  $mchid - 10000;
        //用户信息
        $this->merchants = D('Common/Member')->getById($user_id);
        ...
        $bankname = I("request.bankname", '', 'trim');
        if(!$bankname) {
            $this->showmessage('银行名称不能为空！');
        }
        $subbranch = I("request.subbranch", '', 'trim');
        if(!$subbranch) {
            $this->showmessage('支行名称不能为空');
        }
        $accountname = I("request.accountname", '', 'trim');
        if(!$accountname) {
            $this->showmessage('开户名不能为空！');
        }
        $cardnumber = I("request.cardnumber", '', 'trim');
        if(!$cardnumber) {
            $this->showmessage('银行卡号不能为空！');
        }
        $province = I("request.province", '', 'trim');
        if(!$province) {
            $this->showmessage('省份不能为空！');
        }
        $city = I("request.city", '', 'trim');
        if(!$city) {
...
//                $data = array('status' => 'success', 'msg' => '提现申请成功', 'transaction_id'=>$data['trade_no']);
//                echo json_encode($data);
                $this->showOk("提现申请成功", ['transaction_id'=>$data['trade_no']]);
                exit;
            } else {
                $this->showmessage('系统错误');
            }
        } else {
            $this->showmessage('签名验证失败', $_REQUEST);
        }
    }
